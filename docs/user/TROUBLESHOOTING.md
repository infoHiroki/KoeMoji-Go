...
### 問題が発生した場合は、まず診断を実行してください

KoeMoji-Goには環境診断機能が搭載されています。問題が発生した場合、まず診断を実行することで原因を特定できます。

#### ダブルクリックで診断（推奨）

**Windows**:
1. KoeMoji-Goのフォルダを開く
2. `診断実行.bat` をダブルクリック
3. 診断が自動的に実行される
4. `診断結果.txt` が生成される
...
2. **オーディオデバイス**
   - 全デバイスの列挙（入力/出力）
   - 仮想デバイスの検出（VoiceMeeter、Stereo Mix、BlackHole等）
   - サンプルレート、チャンネル数

3. **デュアル録音対応**
   - Windows: WASAPI/COM利用可否
   - macOS: ScreenCaptureKit利用可否（macOS 13+）
...
問題が解決しない場合、`診断結果.txt` をサポートに送付してください：
...
解決策:
1. v1.8.3以降にアップグレード（推奨）
   - CPU使用を無条件で強制
   - GPU問題を完全回避
   - 期待される処理時間: 1時間の音声 → 20-30分

2. 処理時間の確認方法
   - koemoji.logを開く
   - 「Transcription completed in」で検索
   - 例: "Transcription completed in 25m30s"

3. v1.8.2以前を使用している場合
   - compute_type を "int8" に設定
   - ただし、環境によっては効果がない場合あり
```
...
### Q: "出力ファイルが空です（0バイト）" エラーが出る（v1.8.2+）
```
A: データ消失防止機能により、元ファイルが保護されています
- 原因: 音声/動画ファイルが破損している、または音声認識エンジンが処理できない形式
- 自動保護: 元ファイルはinputフォルダに自動的に保持されます
- 確認事項:
  1. 音声/動画ファイルが正常に再生できるか確認
  2. 破損の可能性: 他のプレーヤーで開けるか確認
  3. 形式変換: WAV形式に変換してから再度処理を試す

- 変換方法例（FFmpegを使用）:
  ffmpeg -i 元ファイル.m4a 変換後.wav

- v1.8.2での改善:
  - これまで: 0バイト出力でも「成功」と判断し、元ファイルがarchiveに移動してデータ消失
  - 改善後: 0バイト検出でエラーを返し、元ファイルはinputフォルダに保持

注意: スマートフォンで録音したAACファイルは破損しやすい傾向があります
```
