...
# 解凍
cd ../releases
tar -xzf koemoji-go-macos-1.8.4.tar.gz
cd koemoji-go-1.8.4

# バージョン確認
./koemoji-go --version
# 期待される出力: KoeMoji-Go v1.8.4
...
```bash
# アプリに実行権限付与
xattr -cr koemoji-go-1.8.4/

# または個別に
xattr -d com.apple.quarantine koemoji-go-1.8.4/koemoji-go
```
...
1. **GitHub Releases にアップロード**
   ...
   # アセットとしてアップロード: koemoji-go-macos-1.8.4.tar.gz
   ```
