...
# 解凍
cd ../releases
tar -xzf koemoji-go-macos-1.8.2.tar.gz
cd koemoji-go-1.8.2

# バージョン確認
./koemoji-go --version
# 期待される出力: KoeMoji-Go v1.8.2
...
```bash
# アプリに実行権限付与
xattr -cr koemoji-go-1.8.2/

# または個別に
xattr -d com.apple.quarantine koemoji-go-1.8.2/koemoji-go
```
...
1. **GitHub Releases にアップロード**
   ...
   # アセットとしてアップロード: koemoji-go-macos-1.8.2.tar.gz
   ```
