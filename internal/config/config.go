...
type Config struct {
	...
	MicrophoneVolume      float64 `json:"microphone_volume"`        // 0.0-1.0, default 1.0
}
...
func GetDefaultConfig() *Config {
	return &Config{
		...
		DualRecordingEnabled:  runtime.GOOS == "windows", // Enabled by default on Windows, disabled on macOS (requires screen recording permission)
		SystemAudioVolume:     0.3,   // Windows only: 0 on system scale (0.1, 0.2, 0.3, 0.5, 0.7)
		MicrophoneVolume:      1.6,   // Windows only: 0 on microphone scale (1.0, 1.3, 1.6, 1.9, 2.2)
	}
}
...
func LoadConfig(configPath string, logger *log.Logger) (*Config, error) {
	...
	if err := json.NewDecoder(file).Decode(config); err != nil {
		...
		return nil, fmt.Errorf("failed to parse config from %s: %v", foundPath, err)
	}
...
func configureRecordingDevice(config *Config, reader *bufio.Reader) bool {
	...
	fmt.Printf("\n%s %s: %s\n", msg.Current, msg.RecordingDeviceName, config.RecordingDeviceName)
	fmt.Println("\n利用可能な録音デバイス:")

	for i, device := range inputDevices {
		fmt.Printf("%d. %s", i+1, device.Name)
		if device.Name == config.RecordingDeviceName {
			fmt.Printf(" (%s)", msg.Current)
		}

		// Check for virtual devices
		name := strings.ToLower(device.Name)
		if strings.Contains(name, "blackhole") || strings.Contains(name, "stereo mix") || strings.Contains(name, "virtual") || strings.Contains(name, "aggregate") {
			fmt.Printf(" [仮想デバイス]")
		}
		fmt.Println()
	}
