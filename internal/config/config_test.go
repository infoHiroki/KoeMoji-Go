...
func TestLoadConfig_ValidFile(t *testing.T) {
	...
	assert.Contains(t, config.InputDir, "test_input")
	assert.Contains(t, config.OutputDir, "test_output")
	assert.Contains(t, config.ArchiveDir, "test_archive")
...
func TestCompleteConfigRoundTrip(t *testing.T) {
	...
	// Path resolution affects loaded config, so check that paths contain expected directory names
	assert.Contains(t, loadedConfig.InputDir, "custom_input")
	assert.Contains(t, loadedConfig.OutputDir, "custom_output")
	assert.Contains(t, loadedConfig.ArchiveDir, "custom_archive")
