...
### v1.8.5での変更（2025-11-06）
1. **macOS TUIモード起動スクリプト追加**
   - `KoeMoji-Go-TUI.command` をダブルクリックでTUIモード起動可能
   - わかりやすいメッセージ表示とUX改善
   - ターミナル操作に不慣れなユーザーでも簡単にTUIモード使用可能

2. **設計判断の記録**
   - 当初はGUIモード起動スクリプトを計画
   - `.command`スクリプトではターミナルウィンドウをプログラム的に閉じることが不可能
   - GUIモードはターミナルウィンドウが残る問題でUX悪化
   - TUIモード専用に方針転換（ターミナルが必要なので自然）

3. **実装内容**
   - `build/macos/KoeMoji-Go-TUI.command` (380バイト)
   - 起動時・終了時に明確なメッセージ表示
   - 終了後に「このウィンドウを閉じてください」と指示
   - `build/macos/build.sh` にパッケージング処理追加

4. **ドキュメント更新**
   - `README.md` - macOS起動方法更新（ダブルクリック方法追加）
   - `build/common/assets/README_RELEASE.md` - 配布版README更新
   - `docs/design/LAUNCH_COMMAND.md` - 完全な設計書作成
